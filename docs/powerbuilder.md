...
All of the PBNI Libraries have Wrapper Class on the PowerBuilder side, because:
 ...
 - To wrap errors with valid stacktraces

To make the Wrapper class its preferred to use Composition instead of Inheritance, because:
...
Though it can still be useful to inherit the PBNI Class while developing, so you don't have to copy all the Method Signatures.
